...
def test_daily_rebalance(
    ...
    sim_engine = BusinessDaysSimulationEngine(sd, ed, pre_market, post_market)

    for sim_events in zip(sim_engine, expected_events):
        calculated_event = sim_events[0]
        expected_event = SimulationEvent(
            pd.Timestamp(sim_events[1][0], tz=pytz.UTC), sim_events[1][1]
        )
